...
function _getDelegates (context, limit = constants.Delegates.ACTIVE_DELEGATES, offset = 0, votes = []) {
  tysApi.getDelegates(limit, offset).then(response => {
    if (response.success) {
...
function checkUnconfirmedTransactions (context) {
  tysApi.checkUnconfirmedTransactions().then(response => {
    if (response.success) {
      if (response.count === 0) {
...
export default {
  ...
  getDelegates (context, payload) {
    tysApi.getDelegatesWithVotes(payload.address).then(response => {
      if (response.success) {
        const votes = response.delegates.map(vote => vote.address)
        tysApi.getDelegatesCount().then(response => {
          if (response.success) {
  ...
  voteForDelegates (context, payload) {
    ...
    tysApi.voteForDelegates(payload.votes).then(response => {
      if (response.success) {
  ...
  getForgingTimeForDelegate (context, delegate) {
    tysApi.getNextForgers().then(response => {
      if (response.success) {
        ...
        tysApi.getBlocks().then(response => {
          if (response.success) {
  ...
  getForgedByAccount (context, delegate) {
    tysApi.getForgedByAccount(delegate.publicKey).then(response => {
      if (response.success) {
        context.commit('update_delegate', {
          address: delegate.address,
