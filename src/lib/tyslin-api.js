...
import utils from './tyslin'
import client from './tyslin-api-client'
 ...
 * @param {object} transaction TYS transaction
...
export function getCurrentAccount () {
  ...
  return client.get('/api/accounts', { publicKey })
    ...
    .then(account => {
      account.balance = utils.toTys(account.balance)
      account.unconfirmedBalance = utils.toTys(account.unconfirmedBalance)
 ...
 * @param {string} address TYS address
 ...
 * @property {number} amount TYSs to send with this message
 */
...
/**
 * Stores the supplied value into the Tyslin KVS under the specified key.
...
/**
 * Retrieves the stored value from the Tyslin KVS
...
/**
 * Sends the specified amount of TYS to the specified TYS address
 * @param {string} to receiver address
 * @param {number} amount amount of TYS to send
 * @returns {Promise<{success: boolean, transactionId: string}>}
 */
