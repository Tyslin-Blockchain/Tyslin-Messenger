...
export default {
  ...
  getNewTransactions (context) {
    ...
    return tysApi.getTransactions(options).then(response => {
  ...
  getTransaction (context, { hash }) {
    return tysApi.getTransaction(hash).then(
  ...
  sendTokens (context, options) {
    return tysApi.sendTokens(options.address, options.amount).then(result => {
