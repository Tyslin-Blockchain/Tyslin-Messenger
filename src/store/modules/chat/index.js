 ...
 *   readOnly?: boolean // for Tyslin Bounty & Tyslin Tokens chats
 * }
 *
 */
const state = () => ({
...
const mutations = {
  ...
  /**
   * Add `Tyslin Bounty` & `Tyslin Tokens` to state.chats.
   */
  createTyslinChats (state) {
    const bountyMessages = [
      {
        id: 'b1',
        message: 'chats.welcome_message',
        timestamp: EPOCH,
        senderId: 'Tyslin Support',
        type: 'message',
        i18n: true,
        status: TS.DELIVERED
      }
    ...
    Vue.set(state.chats, 'Tyslin Support', {
...
const actions = {
  ...
  createChat ({ commit }, { partnerId, partnerName = '' }) {
    if (!validateAddress('TYS', partnerId)) {
      return Promise.reject(new Error('Invalid user address'))
    }

    return tysApi.getPublicKey(partnerId)
      .then(key => {
