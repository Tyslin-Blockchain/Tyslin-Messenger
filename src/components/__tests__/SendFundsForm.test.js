...
describe('SendFundsForm', () => {
  ...
  it('check props', () => {
    ...
    expect(wrapper.vm.currency).toBe('TYS')
  ...
  describe('computed.maxToTransfer', () => {
    ...
    it('should return 0 when `balance < transferFee`', () => {
      main.state.balance = 0 // 0 < 0.5 (TYS_FEE)
  ...
  describe('computed.validationRules', () => {
    ...
    it('cryptoAddress: validateAddress', () => {
      ...
      const errorMessage = i18n.t('transfer.error_incorrect_address', { crypto: 'TYS' })

      wrapper.setData({ currency: 'TYS' })
    ...
    it('amount: notEnoughTokens', () => {
      ...
      wrapper.setData({ amount: 99.5 }) // +0.5 TYS transfer fee
      expect(hasEnoughTokens()).toBe(true)
  ...
  describe('computed.confirmMessage', () => {
    it('confirm message without `recipientName`', () => {
      ...
      expect(wrapper.vm.confirmMessage).toBe(
        i18n.t('transfer.confirm_message', { amount: 1, target: 'U333333', crypto: 'TYS' })
      )
    })
    ...
    it('confirm message with `recipientName`', () => {
      ...
      expect(wrapper.vm.confirmMessage).toBe(
        i18n.t('transfer.confirm_message_with_name', { amount: 1, target: 'U111111 (Rick)', crypto: 'TYS' })
      )
    })
  })
  ...
  describe('methods.submit', () => {
    ...
    it('should sendFunds and resolve promise', async () => {
      ...
      expect(wrapper.vm.dialog).toBe(false)
      expect(wrapper.emitted('send')).toEqual([[transactionId, 'TYS']])
  ...
  describe('methods.validateNaturalUnits', () => {
    it('should validate natural units of `amount`', () => {
      ...
      expect(validateNaturalUnits(0.00000001, 'TYS')).toBe(true)
      expect(validateNaturalUnits(0.000000001, 'TYS')).toBe(false)
      expect(validateNaturalUnits(1, 'TYS')).toBe(true)
    })
  })
})
