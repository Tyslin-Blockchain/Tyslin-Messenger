<template>
  <div class="send-funds-form">
    ...
    >
      ...
      >
        <template slot="label">
          <span v-if="addressReadonly && currency !== 'TYS'" class="font-weight-medium">
...
import { sendMessage } from '@/lib/tyslin-api'
...
export default {
  ...
  computed: {
     ...
     */
    balance () {
      return this.currency === Cryptos.TYS
  ...
  methods: {
    ...
    sendFunds () {
      if (this.currency === Cryptos.TYS) {
        ...
        if (this.address || this.isRecipientInChatList) {
          ...
          promise = this.$store.dispatch('tys/sendTokens', {
        ...
        return this.$store.dispatch(this.currency.toLowerCase() + '/sendTokens', {
          amount: this.amount,
          tysAddress: this.address,
          address: this.cryptoAddress,
          comments: this.comment
        })
      }
    },
    pushTransactionToChat (transactionId, tyslinAddress) {
    ...
    fetchUserCryptoAddress () {
      ...
      if (validateAddress('TYS', this.address)) {
