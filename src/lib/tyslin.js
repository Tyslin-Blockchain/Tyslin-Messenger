...
/**
 * Converts provided `time` to Tyslin's epoch timestamp
...
/**
 * Converts TYS epoch timestamp to a Unix timestamp
...
/**
 * Calculates TYS address based on the public key specified
 * @param {any} publicKey public key to derive TYS address from
 * @returns {string}
 */
...
/**
 * Encodes a text message for sending to TYS
...
/**
 * Converts TYS amount to its internal representation
 * @param {number|string} tysAmount amount to convert
 * @returns {number}
 */
adamant.prepareAmount = function (tysAmount) {
  return Math.round(Number(tysAmount) * 100000000)
}

adamant.toTys = function (rawAmount) {
